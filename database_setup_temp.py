import psycopg2
import datetime
from comparator_temp import main
import time 
import re
...
def create_database(con,cursor):
	...
	create_table_time_query = '''CREATE TABLE Resolve_time
	(created_at date,
	domain VARCHAR(10000) NOT NULL,
	quad_9 numeric,
	quad_9_noblock numeric,
	google numeric,
	cloudflare_safe numeric,
	cloudflare numeric,
	opendns numeric,
	ultrarecursive numeric);'''
	...
	con.commit()
	cursor.execute(create_table_time_query)
	con.commit()
...
def delete_database(con,cursor):
	...
	try:
		cursor.execute("DROP TABLE Resolve_time;")
	except psycopg2.errors.UndefinedTable:
		print("Table Domains doesn't exist")
	con.commit()
